...
AudioEffectGlitch::AudioEffectGlitch() :
	m_delay_buffer(),
...
void AudioEffectGlitch::process_audio_in_impl( int channel, const int16_t* sample_data, int num_samples )
{

	m_delay_buffer.write_to_buffer( sample_data, num_samples );
}

void AudioEffectGlitch::process_audio_out_impl( int channel, int16_t* sample_data, int num_samples )
{

	m_play_heads[channel].read_from_play_head( sample_data, num_samples );
}

int AudioEffectGlitch::num_input_channels() const
{
	return 1;
}

int AudioEffectGlitch::num_output_channels() const
{
	return NUM_PLAY_HEADS;
}

void AudioEffectGlitch::update()
{
...
void AudioEffectGlitch::set_bit_depth( int sample_size_in_bits )
{
	m_next_sample_size_in_bits = sample_size_in_bits;
	//set_bit_depth_impl( sample_size_in_bits );
}

void AudioEffectGlitch::set_loop_moving( bool moving )
{
	m_next_loop_moving = moving;
}

void AudioEffectGlitch::set_loop_size( int play_head, float loop_size )
{
	m_loop_size_ratio[play_head] = loop_size;
}

void AudioEffectGlitch::set_jitter( int play_head, float jitter )
{
	m_jitter_ratio[play_head] = jitter;
}

void AudioEffectGlitch::set_beat()
{
	m_next_beat = true;
}

void AudioEffectGlitch::set_freeze_active( bool active )
{
	m_next_freeze_active = active;
}

int AudioEffectGlitch::num_heads() const
{
	return NUM_PLAY_HEADS + 1; // + 1 for write head
}

void AudioEffectGlitch::head_ratio_details( int head, float& loop_start, float& loop_end, float& current_position ) const
{
